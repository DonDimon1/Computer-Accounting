...
void computerData::on_pushButton_Refresh_clicked() {        // Обновление характеристик о комплектующих ПК
    delete comp; // Очищаем старые данные. С nullptr это безопасно
...
void computerData::readPCCharacteristics(UINT ID) { // Считываем характеристики ПК
    ...
    #ifdef _WIN32
        comp = new InfoWin(this);   //Указатель на класс InfoWin для определения всех характеристик компьютера на Windows
    #elif __linux__
        comp = new InfoLin(this);   //Указатель на класс InfoWin для определения всех характеристик компьютера на Linux
    #else
    ...
    basicInfModel->setData(basicInfModel->index(newRowIndex, basicInfModel->fieldIndex("bitWidth")), "3"/*comp->GetOsBitWidth()*/);         // Получаем разрядность ОС
    basicInfModel->setData(basicInfModel->index(newRowIndex, basicInfModel->fieldIndex("User")), "4"/*comp->GetUserName_()*/);              // Получаем имя пользователя
